    ...
    dbc.Card(
        ...
        children=[
            ...
            dbc.CardHeader(
                dbc.Row(
                    children=[
                        #dbc.Col("Viewer", style={ "font-size": "larger", "font-weight": "bold"}, md=2),
                        dbc.Col(dbc.Row(children=[dbc.Label("FILE PATH: ", style={ "font-size": "larger", "font-weight":"bold"}),
                                                  html.Div(id='show_file_path', style={ "font-size": "larger"})
                                                  ]
                                        )
                                , md=5),
                        dbc.Col(dbc.Row(children=[dbc.Label("FILE NAME:", style={ "font-size": "larger", "font-weight":"bold"}),
                                                  html.Div(id='show-image-name', style={ "font-size": "larger"})
                                                  ]
                                        )
                                , md=5),
                        dbc.Col(dbc.Row(children=[dbc.Label("Nº:", style={ "font-size": "larger", "font-weight":"bold"}),
                                                  html.Div(id='show-page-index', style={ "font-size": "larger"})
                                                  ]
                                        )
                                , md=2),
                    ]
        ...
        children=[
                    ...
                    [
                        ...
                        dcc.RadioItems(
                            ...
                            options=[
                                ...
                                {'label': 'Normal', 'value': 'HEALTHY', 'disabled': True},
                            ],
                            value='HEALTHY',
            ...
            dbc.CardBody(
                [
                    dbc.Row(
                        children=[
                            dbc.Col(dbc.Label("Indicator", style={"font-weight": "bold", 'text-align': 'center', "font-size" : "larger"}), md=8),
                            dbc.Col(
                                dbc.Row(
                                    children=[
                                        dbc.Label("Score :  ", style={"font-weight": "bold", 'text-align': 'center', "font-size" : "larger"}),
                                        dbc.Label(id="total_score",children=[" 7 "], style={ 'text-align': 'center', "font-size" : "larger"})
                                    ],
                                    align= 'center',
                                ),
                            align= 'center',md=4),
...
meta = [
    html.Div(
        ...
        children=[
            ...
            dcc.Store(id='score-memory', data=(0,0,0), storage_type='session'),
            dcc.Store(id='image_saved_data',data= {"shapes": [], "priorities": [(0,0,0)], "lobes": [' '], "cellstatus": [' ']}, storage_type='session'),
...
@app.callback(
    [Output("score-cytoplasm", "value"),
     Output("score-chromatin", "value"),
     Output("score-lobes", "value"),
     Output("cell-condition", "value"),
     Output("lobe-number", "value"),
     Output("lobe-number-normal", "value"),
     Output("score-memory", "data"),
     Output("total_score","children"),

    ],
    [Input("score-cytoplasm", "value"), # score value of cytoplasm
     Input("score-chromatin", "value"), # score value of chromatin
     Input("score-lobes", "value"), # score value of lobes
     Input("image_saved_data", "data"),
     Input("cell-condition", "value"),
     Input("lobe-number", "value"),
     Input("lobe-number-normal", "value"),

    ],
    [State("score-memory", "data"),
     State("cell-condition", "value"),
     State("lobe-number", "value"),
     State("lobe-number-normal", "value"),
    ]
)
def update_directory(value_cytoplasm, value_chromatin, value_lobes, image_saved_data,cellcondition,lobenumber,lobenumbernormal,
                     priority_memory,cell_condition,lobe_number, lobe_number_normal):
    ...
    if 'image_saved_data' not in changed_id:
        ...
        if ('lobe-number-normal' in changed_id) and (lobe_number_normal=='Normal') :
            lobe_number = 'Normal'
        elif ('lobe-number' in changed_id) and ((lobe_number=='Hypersegmented') or (lobe_number=='Hyposegmented')):
            lobe_number_normal = ' '
        print(lobe_number)
        '''
        if 'score-cytoplasm' in changed_id:
            changed_value=feature_name[1]
            new_priority=value_cytoplasm
        elif 'score-chromatin' in changed_id:
            changed_value = feature_name[2]
            new_priority = value_chromatin
        elif 'score-lobes' in changed_id:
        ...
        '''
        if (lobe_number == "Hyposegmented") or (lobe_number == "Hypersegmented")  :
            value_lobes=1
        else:
            value_lobes = 0
        score_memory = (value_cytoplasm, value_chromatin, value_lobes)
    else:
        cell_condition = image_saved_data['cellstatus']
        cell_condition= cell_condition[0]
        lobe_number = image_saved_data['lobes']
        lobe_number= lobe_number[0]
        lobe_number_normal= lobe_number
        print(lobe_number[0])
        score_memory = image_saved_data['priorities']
        #print('score_memory', score_memory)
        if isinstance(score_memory[0],list):
          score_memory= score_memory[0]
        #print('score_memory',score_memory)
        value_cytoplasm = score_memory[0]
        value_chromatin = score_memory[1]
        value_lobes = score_memory[2]



    total_score=score_memory[0]+score_memory[1]+score_memory[2]
    #print(changed_id)
    #print('here',value_cytoplasm, value_chromatin, value_lobes, priority_memory, lobe_number,cell_condition)
    return value_cytoplasm, value_chromatin, value_lobes, cell_condition,lobe_number,lobe_number_normal, score_memory,str(total_score)
...
@app.callback(
    [Output("output-path", "data"),
     ...
     Output("score-cytoplasm", "options"),
     Output("score-chromatin", "options"),
     Output("score-lobes", "options"),
     Output("cell-condition", "options"),
     Output("lobe-number", "options"),
     Output("lobe-number-normal", "options"),
     ],
...
def select_directory(n1):
    ...
    options_lobes_score_F = [
        {'label': '0', 'value': 0, 'disabled': False},
        {'label': '1', 'value': 1, 'disabled': False},
        #{'label': '2', 'value': 2, 'disabled': False},
        #{'label': '3', 'value': 3, 'disabled': False},
    ]
    options_lobes_score_T = [
        {'label': '0', 'value': 0, 'disabled': True},
        {'label': '1', 'value': 1, 'disabled': True},
        #{'label': '2', 'value': 2, 'disabled': True},
        #{'label': '3', 'value': 3, 'disabled': True},
    ...
    options_lobes_normal_T = [
        {'label': 'Normal (3-5)', 'value': 'Normal', 'disabled': True},
        ]
    options_lobes_normal_F = [
    ...
    options_cell_F = [
                  ...
                  {'label': 'Normal', 'value': 'HEALTHY', 'disabled': False},
              ]
    if n1:
        ...
        if os.path.isdir(image_path):
            ...
            return image_path, False, False, False, False, False, False, False, options_F,options_F,options_lobes_score_F, options_cell_F, options_lobes_F, options_lobes_normal_F
        else:
            return dash.no_update
    else:
        return DEFAULT_IMAGE_PATH, True, True, True, True, True, True, True, options_T,options_T,options_lobes_score_T, options_cell_T, options_lobes_T, options_lobes_normal_T
...
@app.callback(
    Output("show_file_path", "children"),
...
@app.callback(
    [Output("show-image-name", "children"),
     Output("show-page-index", "children"),
...
def change_image(next_b, prev_b, path_2, stroke_width_value, graph_relayoutData,
                 transparent_but, granular_but, heterogenic_but, homogenic_but,cell__condition,lobe__number,lobe__number_normal,priority__memory,
                 i, data, path, masks_data, figure, mark_flag, stroke_color_memory, save_image, cell_condition, priority_list, lobe_number, lobe_number_normal, image_saved_data):
    ...
    if 'next-image-button' in changed_id:
        ...
        if os.path.isfile(json_mask_data):  # file with notation shapes exists
            ...
            masks_data = {"shapes": [], "priorities": [(0,0,0)], "lobes": [' '], "cellstatus": [' ']}
        ...
        return data[image_index], (str(return_val) + '/'+ str(len(data))), image_index, fig_out, masks_data, mark_flag, stroke_color_memory,image_saved_data
        ...
        if changed_id == "graph.relayoutData":
            if "shapes" in graph_relayoutData.keys():
                ...
                return dash.no_update
        if ("cell-condition" in changed_id) or ( "lobe-number" in changed_id) or ("score-memory" in changed_id):
            mark_flag = True
            print(mark_flag)
        ...
        return_val = copy.deepcopy(image_index)
        return_val= return_val+1
        return data[image_index], (str(return_val) + '/'+ str(len(data))), image_index, fig_out, masks_data, mark_flag, stroke_color_memory,dash.no_update
